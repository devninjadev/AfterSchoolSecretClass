...
두 뷰 모두 공식 Notion 데이터베이스 조회에서 `data.mode=view`, `is_archived:false`, `page_size:100`을 사용한다. 저장 뷰의 필터·정렬·표시 속성을 유지한다. 사용자 질의와 관련된 현재 Story와 최근 Report만 읽고, 전체 내용을 무조건 프롬프트에 싣지 않는다.

## 읽기 절차

1. 정확한 Hub와 계약 표식을 확인한다.
2. `Stories Current` 저장 뷰에서 미해결 현재 가설을 읽는다.
3. 필요할 때 `Reports Recent`에서 관련 최근 보고서를 읽는다.
4. 각 레코드의 관찰·생성 시각과 신뢰도, 가설, 반대 근거, 무효화 조건을 보존한다.
5. 현재 뉴스·시장·상품 증거와 비교해 `strengthened`, `weakened`, `maintained`, `unresolved` 중 하나로 분류한다.
6. World Memory와 현재 근거가 충돌하면 현재 사실을 덮어쓰지 말고 충돌을 설명한다.

정확한 저장 뷰 읽기가 실패하면 같은 뷰에 한 번만 읽기 재시도한다. 두 번째 실패 뒤에는 World Memory 없이 계속한다. 제목 검색, 광범위 의미 검색, 다른 Hub 추측, SQL, 스키마 수리, 마이그레이션으로 우회하지 않는다.

## 쓰기 경계

자동 읽기는 허용되지만 자동 쓰기는 허용되지 않는다. `참고해`, `기억해 둬`, `다음에도 고려해`는 쓰기 승인이 아니다.

다음처럼 명시적인 요청만 쓰기 워크플로 입구가 될 수 있다.
...
그 경우에도 하영이 직접 변형하지 않고 `$world-memory-autopilot`의 확인·중단·쓰기 영수증 계약에 위임한다. 명시적 쓰기 의도가 없으면 Collection, Report, Story, Story Change의 생성·수정과 일정 실행, 설정, 수리, 이동, 삭제, 스키마 변경을 모두 금지한다.

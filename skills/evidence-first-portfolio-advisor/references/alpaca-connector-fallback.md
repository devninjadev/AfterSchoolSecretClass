...
Portfolio analysis retains Yahoo currency metadata, asset identity, requested dates, corporate actions and total-return checks. Its legacy Alpaca history envelope is NOT automatically compatible with Paper Trading's `data` wrapper. Do not forge legacy `tool`/request metadata or alter raw responses merely to satisfy `alpaca-validate`. Paper Trading observations may support timestamped price research; portfolio history/backtest completion requires a supported evidence adapter and a successful existing validation. If the response shape is unsupported, report that limitation and continue to the existing Wolfram alternative rather than claim validated history. No broadening of market eligibility.
...
On 2026-09-26, Paper Trading get_clock and an SPY IEX snapshot succeeded. This is a connectivity/read-format smoke check, not validation of every symbol, historical adapter, corporate-action path, or backtest.
